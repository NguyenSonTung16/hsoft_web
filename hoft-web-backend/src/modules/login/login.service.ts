...
export async function loadFacilities(): Promise<FacilityOption[]> {
  const rows = await repository.loadFacilities();
...
export async function loadLabAreas(facilityId: string): Promise<LabAreaOption[]> {
  const rows = await repository.loadLabAreas(facilityId);
  return rows.map((row) => ({ id: row.ID, code: row.CODE, name: row.NAME }));
}
...
export async function submitLogin(
  ...
  try {
    const userRow = await repository.findUserByUsername(input.username);
    ...
    if (!passwordValid) {
      await repository.incrementFailedAttempts(userRow.ID);
      await logSubmitErrorBestEffort(
        input.username,
        input.facilityId,
        input.labAreaId,
    ...
    const hasScopeAccess = await repository.hasScopeAccess(
      userRow.ID,
      input.facilityId,
      input.labAreaId
    );
    ...
    const sessionId = await repository.submitLoginTransactionViaProcedure({
      sessionToken,
      userId: userRow.ID,
...
export async function refreshSession(
  ...
  const refreshed = await repository.refreshSession(sessionToken);
  if (!refreshed) {
    logger.warn("login.refresh.expired", "Refresh session failed: token invalid or expired", {
      sessionToken,
    });
    return null;
  }
